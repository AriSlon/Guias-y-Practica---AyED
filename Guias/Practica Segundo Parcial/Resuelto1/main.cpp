...
ostream& operator << (ostream& os, Nivel1 n1) 
{
	os << n1.pais << "\t" << n1.empresa << "\t" << n1.totenerg << "\t" 
	   << n1.cantdias << "\t" << n1.totenerg / n1.cantdias;
	if (n1.totenerg / n1.cantdias > minprom) {
		os << endl;
		mostrar(n1.lista);
	}
	return os;
...
bool mayor_que_siguiente (Nodo<Nivel1> *actual)
{
	...
	float prom_actual = actual->dato.totenerg / actual->dato.cantdias;
	actual = actual->sig;
	float prom_sig = actual->dato.totenerg / actual->dato.cantdias;
	return prom_actual > prom_sig;
}
